...
class DBAPI:
    ...
    def insert_booked_ticket(self, publicKey, emailHash, nameHash, date):
        if not self.__check_date(date):
            return False
        
        self.cursor.execute("INSERT into BookedTickets (publicKey, emailHash, nameHash, date)values (%s,%s,%s,%s) ", (publicKey, emailHash, nameHash, date))
        self.ticketdb.commit()
...
if __name__ == "__main__":
    ...
    ticket_list = [("29.06.2021", 100), ("28.06.2021", 105)]
    #testdb.insert_tickets(ticket_list)
    testdb.get_tickets_by_date("26.06.2021")
    print(testdb.insert_booked_ticket("publicKey", "jhashdjashd", "jjahsdjha", "25.06.2021"))
    print(testdb.query_all("jjahsdjha", "jhashdjashd", "26.06.2021", 1))
    testdb.del_entry("jjahsdjha", "jhashdjashd", "25.06.2021", 2)